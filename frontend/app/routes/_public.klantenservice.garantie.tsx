...
import { getWarrantyPage } from "../core/modules/SingleTypes/warranty/api";
...
export const meta: MetaFunction = () => {
  return [
    { title: "Klantendienst - Garantie | Fixit Aalst" },
    {
      name: "description",
      content:
...
export async function loader() {
  try {
    const warranty = await getWarrantyPage();

    if (!warranty || !warranty.data) {
...
export default function Warranty() {
  const { warranty } = useLoaderData() as LoaderData;

  if (!warranty || !warranty.PageContent) {
    return (
