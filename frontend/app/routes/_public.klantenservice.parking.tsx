...
import { getParkingPage } from "../core/modules/SingleTypes/parking/api";
...
export const meta: MetaFunction = () => {
  return [
    { title: "Klantendienst - Parking | Fixit Aalst" },
    {
      name: "description",
      content:
...
export async function loader() {
  try {
    const parking = await getParkingPage();
...
export default function Parking() {
  const { parking } = useLoaderData() as LoaderData;

  if (!parking || !parking.PageContent) {
    return (
  ...
  return (
    <div className="space-y-6 px-5 md:px-10 lg:px-32 w-2/3 mx-auto">
      {parking.PageContent.map((block: any) => {
