...
import { getWhichDevicePage } from "../core/modules/SingleTypes/whichDevice/api";
...
export const meta: MetaFunction = () => {
  return [
    { title: "Klantendienst - Welk toestel? | Fixit Aalst" },
    {
      name: "description",
      content:
...
export async function loader() {
  try {
    const whichDevice = await getWhichDevicePage();

    if (!whichDevice || !whichDevice.data) {
      throw new Error("Geen data beschikbaar");
    }

    return {
      whichDevice: whichDevice.data,
    };
  } catch (error) {
    console.error(
    ...
    return { whichDevice: null };
  }
}

export default function whichDevice() {
  const { whichDevice } = useLoaderData() as LoaderData;

  if (!whichDevice || !whichDevice.PageContent) {
    return (
